...
class NeoXArgsModel(NeoXArgsTemplate):
    ...
    linear_zero_mean: bool = False
    """
    LinearZeroMean for CRMSNorm
    """
    ...
    norm: Literal["layernorm", "rmsnorm", "scalenorm", "crmsnorm", "jslayernorm"] = "layernorm"
    """
    Normalization layer to use. Choose from "layernorm", "rmsnorm", "scalenorm" , "jslayernorm".
    """
    ...
    """
    crms_norm_epsilon: float = 1.0e-8
    """
    Root mean squared norm epsilon
    """
    jslayer_norm_epsilon: float = 1.0e-8
    """
    James-Stein layer norm epsilon
    """
    ...
    norm_rearrange: bool = False
    """
    If false, we use the conventional residual path:
      x = x + attn(ln1(x))
      x = x + mlp(ln2(x))
    Otherwise, harshit's rearrange of norms:
      x = x + ln1(attn(x))
      x = x + ln2(mlp(x))
    """


    dual_residual: bool = False
    """
    Dual Residual Norm Paper
    """
    ...
    CA_embedding: bool = False
    """
    if true harshit's context aware embeddings
    """

    CAE_window_size: int = 2
    """
    Default window size for context 2 prev, 2 after for 1 in center
    """
