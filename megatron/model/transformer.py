...
class ParallelMLP(nn.Module):
    ...
    def __init__(
        ...
        ff_mult = int(4 * 2 / 3) if self.activation_type in ["swiglu", "geglu"]   else 4
        ff_dim = (
        ...
        ff_dim_in = ff_dim // 2 if self.activation_type in ["swiglu", "geglu"]  else ff_dim
        # Project back to h.
        if neox_args.linear_zero_mean:
            self.dense_4h_to_h = mpu.RowParallelLinearZeroMean(
                neox_args=neox_args,
    ...
    def forward(self, hidden_states):
        ...
        print("\noutput bias  from  h to 4h : \n",bias_parallel)
        if (
            self.activation_type == "gelu" and self.bias_gelu_fusion
        ) or  self.activation_type in ["swiglu", "geglu"]:
        ...
        print("\nactivation output  : \n",intermediate_parallel)
        print("\nactivation output bias  : \n",bias_parallel)

        # [s, b, h]
        output, output_bias = self.dense_4h_to_h(intermediate_parallel)
        print("\noutput  from  4h_to_h mlp: \n",output)
        print("\noutput bias from  4h_to_h mlp: \n",output_bias)
...
class ParallelSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        # Output.
        if neox_args.linear_zero_mean:
            self.dense = mpu.RowParallelLinearZeroMean(
                neox_args=neox_args,
                input_size=neox_args.hidden_size,
                output_size=neox_args.hidden_size,
                input_is_parallel=True,
        ...
        if self.use_cache:
            with torch.no_grad():
                attention_mask = attention_mask[
                    ..., : attention_scores.size(3), : attention_scores.size(0)
                ]
...
class ParallelTransformerLayer(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.dual_residual = neox_args.dual_residual
        self.layer_number = layer_number
        self.num_layers = neox_args.num_layers
        self.neox_args = neox_args

        norm, eps = get_norm(neox_args)

        # Layernorm on the input data.
        # if not self.dual_residual:
        ...
        self.norm_rearrange = neox_args.norm_rearrange
        ...
        # self.norm_rescale = torch.nn.Parameter(torch.randn((2*self.num_layers+1,)))
        ...
        if self.dual_residual:
            self.attn_post_norm = norm(neox_args.hidden_size, eps=eps)
            self.fnn_post_norm = norm(neox_args.hidden_size, eps=eps)
            self.final_pre_norm = norm(neox_args.hidden_size, eps=eps)
    ...
    def forward(self, x, attention_mask, layer_past=None):
        ...
        if self.gpt_j_residual:
            ...
            if self.use_cache:
                ...
                # presents = self.input_layernorm(presents)
                self.layer_past = presents
            else:
                attention_output = self.input_layernorm(attention_output)
            ...
            with torch.enable_grad():
                if attention_bias is not None:
                    # attention_bias = self.input_layernorm(attention_bias)
            ...
            with torch.enable_grad():
                if self.mlp_type == "llama" or self.num_experts > 1:
                    ...
                    # mlp_bias = self.post_attention_layernorm(mlp_bias)
                    output = bias_dropout_fn(
                        mlp_output,
        ...
        elif self.dual_residual:
            # just for 1st layer : (x = x)  and (res = scalar * x) 

            # res = res + (scalar * attn(x))
            # x = ln1(x + attn(x))
            # res = res = (scalar * ffn(x))
            # x = ln2(x + ffn(x))
            
            # just for Last layer : (final_x =  x + ln3(res))

            if self.layer_number == 0: # in the first layer
                res = x*1.0
                # res = x*self.norm_rescale[self.num_layers*2] #the scalar, to be converted to a neox argument
                # print("at layer: ",self.layer_number," norm_rescale is:  ",self.norm_rescale[self.num_layers*2])
            else:
                res = x[1]
                x = x[0]
            ...
            with torch.enable_grad():
                attention_output = (
                    torch.nn.functional.dropout(
                        attention_output if attention_bias is None else  attention_output + attention_bias.expand_as(xr),
                        p=self.hidden_dropout,
                        training=self.training,
                    )
                )
            ###################################################
            res = res + attention_output*1.0
            # res = res + attention_output*self.norm_rescale[(self.layer_number)*2] #the scalar, to be converted to a neox argument
            # print("at layer: ",self.layer_number," norm_rescale is:  ",self.norm_rescale[(self.layer_number)*2] )

            layernorm_output = self.attn_post_norm(attention_output+ xr)
            #########################################################################################
            ...
            with torch.enable_grad():
                mlp_output = (
                    torch.nn.functional.dropout(
                        mlp_output if self.moe_type == "deepspeed" else mlp_output + mlp_bias.expand_as(xr),
                        p=self.hidden_dropout,
                        training=self.training,
                    )
                )
            ###################################################
            res = res + mlp_output*1.0
            # res = res + mlp_output*self.norm_rescale[(self.layer_number)*2 + 1]
            # print("at layer: ",self.layer_number," norm_rescale is:  ",self.norm_rescale[(self.layer_number)*2 + 1] )

            
            
            layernorm_output = self.fnn_post_norm(mlp_output + xr)
            #########################################################################################

            if self.layer_number == self.num_layers-1:
                output = layernorm_output + self.final_pre_norm(res)
                # exit()
            else:
                output = torch.stack([layernorm_output, res], dim=0).to(torch.cuda.current_device())
