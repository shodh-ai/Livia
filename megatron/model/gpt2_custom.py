...
from megatron.model.custom_arch import EmbeddingPipe
from megatron.model.custom_arch import ParallelTransformerLayerPipe
...
def gpt2_attention_mask_func(attention_scores, ltor_mask):
    ...
    # print(attention_scores.shape, ltor_mask.shape, mask_value.shape)
...
def _pre_transformer_block(args):
    ...
    # print(f'inside pre transformer {args[0]}')
...
def _post_transformer_block(args):
    ...
    # print(f'inside post transformer block for args {args[0]}')
    fn = lambda _args: (_args[0].transpose(0, 1).contiguous())
    return fn(args)


class CustomModelPipe(PipelineModule, torch.nn.Module):
    """Custom GPT2Model adapted for pipeline parallelism.
