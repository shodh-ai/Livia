...
class VocabParallelContextAwareEmbedding(VocabParallelEmbedding):
    def __init__(self, neox_args, num_embeddings, embedding_dim, window_size=2, init_method=init.xavier_normal_):
        super().__init__(neox_args, num_embeddings, embedding_dim, init_method)
        self.window_size = window_size
        self.conv = torch.nn.Conv1d(embedding_dim, embedding_dim, kernel_size=(window_size * 2 + 1), padding=(window_size))
        self.pos_embedding_layer = torch.nn.Embedding(neox_args.seq_length, embedding_dim)
    ...
    def forward(self, input_):
        # Get the embeddings from the base class
        token_embeddings = super().forward(input_)

        # Token & positional embeddings
        pos_indices = torch.arange(token_embeddings.shape[1]).to(input_.device)
        # norm = torch.linalg.norm(self.pos_embedding_layer.weight.data,ord=2,dim=-1,keepdim=True)
        # self.pos_embedding_layer.weight.data = (self.pos_embedding_layer.weight.data / norm ) * self.embedding_dim
        pos_embeddings = self.pos_embedding_layer(pos_indices)

        # Apply convolution to get context window
        context_emb = self.conv(token_embeddings.permute(0, 2, 1)).permute(0, 2, 1)

        return context_emb + pos_embeddings
...
class RowParallelLinearZeroMean(torch.nn.Module):
    """Linear layer with row parallelism.
    ...
    def __init__(
        ...
        super(RowParallelLinearZeroMean, self).__init__()
    ...
    def forward(self, input_):
        ...
        output_parallel = F.linear(input_parallel, zero_mean_weight)
        # All-reduce across all the partitions.
        if not self.parallel_output:
            output_ = reduce_from_model_parallel_region(output_parallel)
        else:
        ...
        if not self.skip_bias_add:
            ...
            output = output_
            output_bias = self.bias - self.bias.mean()
            # output_bias = self.bias
        return output, output_bias