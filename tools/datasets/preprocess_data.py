...
def get_args(input_args=None):
    ...
    group.add_argument(
        ...
        type=str,
        required=False,
        help="Path to directory containing input jsonl files or lmd archive(s)",
    )
    group.add_argument(
        "--jsonl-keys",
        nargs="+",
    ...
    group.add_argument(
        "--workers", type=int, default=20, help="Number of worker processes to launch"
    )
    group.add_argument(
        "--log-interval",
        type=int,
...
def main(input_args=None):
    ...
    if args.input_dir != None:
        jsonl_list = glob.glob(args.input_dir + '/**/*.jsonl', recursive=True)
        fin = yield_from_files(jsonl_list, semaphore)
