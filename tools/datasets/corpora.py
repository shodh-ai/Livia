...
class DataDownloader(ABC):
    ...
    def tokenize_custom(self):
        """tokenizes dataset"""
        parent_folder = os.path.join(self.base_dir,self.dataset_name[0])
        jsonl_filepath = os.path.join(parent_folder,f"{self.dataset_name[0]}.zip")

        cmd = f"python tools/datasets/preprocess_data.py \
            --input {jsonl_filepath} \
            --output-prefix {parent_folder}/{self.dataset_name[0]} \
            --vocab {self.vocab_file} \
            --dataset-impl mmap \
            --tokenizer-type {self.tokenizer_type} \
            --merge-file {self.merge_file} \
    ...
    def prepare(self):
        ...
        if "custom" in self.dataset_name:

            # if not os.path.exists(os.path.join(self.base_dir,self.dataset_name[0],"custom.zip")):
            #     text = ""
            #     output_merge_vocab_dir = os.path.join(self.base_dir,self.dataset_name[0])
                
            #     if len(self.datasets)>1:
            #         concat = []
            #         for dataset_name in tqdm(self.datasets):
            #             print(f"Loading {dataset_name}")
            #             dataset = load_dataset(dataset_name,split="train")
            #             concat.append(dataset)

            #         combined_dataset = concatenate_datasets(concat)

            #     else:
            #         combined_dataset = load_dataset(dataset_name,split="train")


                # if not (os.path.exists(os.path.join(self.base_dir,self.dataset_name[0],"vocab.json")) and \
                        
                #         os.path.exists(os.path.join(self.base_dir,self.dataset_name[0],"merges.txt"))):
                    
                #     tokenizer = ByteLevelBPETokenizer()
                #     tokenizer.train_from_iterator(combined_dataset, vocab_size=30000, min_frequency=2, special_tokens=[
                #         "<s>",
                #         "<pad>",
                #         "</s>",
                #         "<unk>",
                ...
                #     # Save the tokenizer
                #     tokenizer.save_model(output_merge_vocab_dir)
                
                # for split in combined_dataset:
                #     if split['question']: text += "###question:" + split['question'] + "\n"
                #     try:
                #         if split['response']: text += "###answer:" + split['response'] + "\n"
                #     except:
                #         if split['answer']:   text += "###answer:" + split['answer'] + "\n"

                # text+="<|endoftext|>"

                # zip_file_path = os.path.join(self.base_dir,self.dataset_name[0])
                # if not os.path.exists(zip_file_path):
                #     os.mkdir(zip_file_path)
                # with zipfile.ZipFile(zip_file_path + "/custom.zip", 'w') as zipf:
